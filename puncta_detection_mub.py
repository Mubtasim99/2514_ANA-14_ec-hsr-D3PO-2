...
features_of_interest = ['granule_cov', 'granule_skew', 'granule_intensity', 'eccentricity', 'cell_intensity_mean','circularity', 'cell_size', 'aspect_ratio', 'area']
...
# biomarker_pairs = [(('hsrMDM2', 'NPM1'),('hsrMDM2', 'SON')), 
#          (('hsrMDM2', 'NPM1'),('hsrMDM2', 'H2AX')),
#          (('hsrMDM2', 'SON'),('hsrMDM2', 'H2AX')),
#          (('ecMDM2', 'NPM1'),('hsrMDM2', 'SON')),
#          (('ecMDM2', 'NPM1'),('hsrMDM2', 'H2AX')),
#          (('ecMDM2', 'SON'),('hsrMDM2', 'H2AX')),
#          ()
#          ]

treatment_pairs = [(('hsrMDM2', '(+)Hygro'),('hsrMDM2', '(-)Hygro')), 
         (('ecMDM2', '(+)Hygro'),('ecMDM2', '(-)Hygro')),
         ]

biomarker_pairs = [(('hsrMDM2', 'NPM1'),('ecMDM2','NPM1')),
 (('hsrMDM2', 'SON'),('ecMDM2','SON')),
 (('hsrMDM2', 'H2AX'),('ecMDM2','H2AX')),
 ()
 ]

# treatment_pairs = [(('hsrMDM2', '(+)Hygro'), ('ecMDM2', '(+)Hygro')),
#                    (('hsrMDM2', '(-)Hygro'), ('ecMDM2', '(-)Hygro')),
#                    ()
#                    ]

order = ['hsrMDM2', 'ecMDM2']
x = 'cell_type' 
for item in [('biomarker', biomarker_pairs), ('treatment', treatment_pairs)]:
    hue = item[0]
    pairs = item[1]
    plt.figure(figsize=(15, 10))
    plt.subplots_adjust(hspace=0.5)
    plt.suptitle(f'{hue}_calculated parameters - per granule, not normalized', fontsize=18, y=0.90)
    for n, parameter in enumerate(features_of_interest):
        # add a new subplot iteratively
        ax = plt.subplot(3, 3, n + 1)

        sns.stripplot(data=granule_summary_reps_df, x=x, y=parameter, hue=hue, dodge='True', edgecolor='k', linewidth=1, size=8, order=order)

        # store legends info
        handles, labels = ax.get_legend_handles_labels()

        sns.violinplot(data=feature_information.reset_index(drop=True), x=x, y=parameter, hue=hue, 
                    palette=['.9'], order=order, ax=ax)
        
        # statannot stats
        annotator = Annotator(ax, pairs, data=granule_summary_reps_df, x=x, y=parameter, hue=hue, order=order)
    ...
    plt.tight_layout()
    plt.legend(handles, labels, bbox_to_anchor=(1.1, 1), title=f'{hue}')
    plt.savefig(f'{output_folder}{hue}_puncta-features_pergranule_raw.png', bbox_inches='tight',pad_inches = 0.1, dpi = 300)
...
for item in [('biomarker', biomarker_pairs), ('treatment', treatment_pairs)]:
    ...
    for n, parameter in enumerate(summary.columns.tolist()[3:-3]):
            ...
            sns.stripplot(data=summary, x=x, y=parameter, dodge='True', hue=hue,
                            edgecolor='k', linewidth=1, size=8, order=order, ax=ax, alpha=0.5)
            # store legends info
            handles, labels = ax.get_legend_handles_labels()
            sns.boxplot(data=summary, x=x, y=parameter, hue=hue,
                        palette=['.9'], order=order, ax=ax)

            # statannot stats
            annotator = Annotator(ax, pairs, data=summary, x=x, y=parameter, hue=hue, order=order)
            annotator.configure(test='t-test_ind', verbose=2)
            annotator.apply_test()
            annotator.annotate()
...
for item in [('biomarker', biomarker_pairs), ('treatment', treatment_pairs)]:
    ...
    for n, parameter in enumerate(summary.columns.tolist()[3:-3]):
        ...
        sns.stripplot(data=normalized_summary, x=x, y=parameter, dodge='True', hue=hue, edgecolor='k', linewidth=1, size=8, order=order, ax=ax)
        # store legends info
        handles, labels = ax.get_legend_handles_labels()
        sns.boxplot(data=normalized_summary, x=x, y=parameter, hue=hue, palette=['.9'], order=order, ax=ax)
        

        # statannot stats
        annotator = Annotator(ax, pairs, data=normalized_summary, x=x, y=parameter, hue=hue, order=order)
        annotator.configure(test='t-test_ind', verbose=2)
        annotator.apply_test()
        annotator.annotate()
